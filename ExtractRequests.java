...
______________________
  ...
  <label><input type="checkbox" id="parseCdata" checked> Parse XML inside CDATA</label>
...
<script>
  // ... keep the rest; just change the fetch URL:
  const unwrapEl = document.getElementById('unwrap');
  const parseCdataEl = document.getElementById('parseCdata');
  const flattenEl = document.getElementById('flatten');
  const coerceEl = document.getElementById('coerce');
